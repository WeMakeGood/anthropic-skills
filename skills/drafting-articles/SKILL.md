...
description: Drafts research-grounded long-form articles across multiple sessions. Reads a project manifest to find voice profile, writing standards, audience document, and research, then runs Orient, Comprehend, Draft, and Editorial phases with process gates. Use when drafting, writing, resuming, or working on an article, or when the user says draft article, resume drafting, write article, draft standalone article, or references an article by number or title. Works for both series articles and standalone pieces.
---
...
|---------|--------|------------------|-------------|
| A | Orient | [PHASE_1_2_ORIENT.md](references/phases/PHASE_1_2_ORIENT.md) | Read manifest, load research, find the story |
| B | Comprehend | [PHASE_3_COMPREHEND.md](references/phases/PHASE_3_COMPREHEND.md) | Think through the evidence |
| C | Draft | [PHASE_4_DRAFT.md](references/phases/PHASE_4_DRAFT.md) | Load voice + standards, write the article |
| D | Editorial + Quality + Present | [PHASE_5_6_7_EDITORIAL.md](references/phases/PHASE_5_6_7_EDITORIAL.md) | Revise, check, deliver |
...
|---------|--------|-------------|
| A | Orient | Read-heavy; Orient needs loaded context fresh |
...
| C | Draft | Voice profile and writing standards must be fresh in context |
...
2. **Read the Phase 1-2 instruction file:** [references/phases/PHASE_1_2_ORIENT.md](references/phases/PHASE_1_2_ORIENT.md)
3. **Begin Phase 1.**
...
| [references/phases/PHASE_1_2_ORIENT.md](references/phases/PHASE_1_2_ORIENT.md) | Orient instructions |
...
| [references/phases/PHASE_5_6_7_EDITORIAL.md](references/phases/PHASE_5_6_7_EDITORIAL.md) | Editorial + Quality + Present instructions |
...
- **Writing about the author instead of as the author.** When the voice profile describes the article's author — the person whose byline the article carries — the agent's default is to write about them in third person, quoting them as a source. The calibration gate captures correct analytical understanding ("Who is this person? Christopher Frazier, a practitioner who...") but the agent never inhabits the role — it writes a journalist's profile piece instead of a first-person practitioner essay. The fix is not editing third person into first person (that produces stitched voice). The fix is the calibration gate requiring first-person commitment: "Your role is now this author. In this role, answer..." If the gate output reads like analysis of the author, the role is not active.
...
- **Drafting sections as adjacent containers rather than parts of a single structure.** The agent's default is to treat each section as a self-contained unit organized by topic. The fix is the structural plan from Comprehend: per-section argument sequences with structural annotations explaining why each move is necessary in its position.

- **Producing table-level structural plans instead of argument sequences.** The structural plan must be detailed enough that a different session can draft from it without re-deriving the argument logic: reader entry point, argument statement, numbered argument sequence, closing handoff.
...
- **Skipping voice and standards loading in cold-start sessions.** Sessions C and D start without voice or standards in memory. The agent's default is to jump to drafting or editing without loading them. The result: guardrails fire but have nothing to enforce against. The phase files enforce this through loading gates — do not skip them.
...
- **Verbose process logs that consume context window.** The log records reasoning, decisions, and self-corrections — not summaries of source material. One sentence per document, one-line loading records, no duplication of plan content.
</failed_attempts>
